...
[cellpose_results_files, cellpose_results_path] = uigetfile({'*.tif'},...
    'Open Images of Segment',...
    'MultiSelect', 'on');

jaccardRange = 0.1:0.1:0.9;
Jaccard_average = zeros(length(cellpose_results_files), length(jaccardRange));
Dice_average = zeros(length(cellpose_results_files), length(jaccardRange));
match_num = zeros(length(cellpose_results_files), length(jaccardRange));
jaccard_countingAccuracy = zeros(length(cellpose_results_files), length(jaccardRange));
dice_countingAccuracy = zeros(length(cellpose_results_files), length(jaccardRange));
false_negative = zeros(length(cellpose_results_files), length(jaccardRange));
false_positive = zeros(length(cellpose_results_files), length(jaccardRange));

% h1 = figure;
% h2 = figure;

for j=1:length(cellpose_results_files)
    j
    GT_img_path_temp=fullfile(gt_path, gt_files{j}); %get input_tiff with absolute path
    GT_img=loadtiff(GT_img_path_temp);    
    
    biofilmQresult_path_temp=fullfile(cellpose_results_path,cellpose_results_files{j}); %get input_tiff with absolute path
...
save('Cellpose_500_1.mat')
title('Cellpose 500 1')